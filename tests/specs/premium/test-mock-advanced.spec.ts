...
test.describe('テスト＆モック：高度なブラウザAPIモックとアサーション詳細表示', () => {
    ...
    test('テスト失敗時の詳細なエラー理由とDOMスナップショット（ダンプ）の表示', async ({ editorPage, editorHelper }) => {
        ...
        await test.step('1. 意図的に失敗するテストシナリオを作成', async () => {
            ...
            const scenarioNameInput = modal.locator('#scenario-name');
            await expect(scenarioNameInput).toBeEditable();
    ...
    test('3種類のブラウザモック（同期、非同期、プロパティ）と関数の連鎖(IIFE)の実行', async ({ editorPage, editorHelper }) => {
        ...
        await test.step('1. 各種ブラウザモックを定義する', async () => {
            ...
            await mockPathInput.fill('navigator.userAgent');
            await mockPatternInput.fill('CustomAgent');
            await mockResponseInput.fill('"PwappyTestBrowser"');
            ...
            await mockPathInput.fill('window.btoa');
            await mockPatternInput.fill('FixedBase64');
            await mockResponseInput.fill('"MOCKED_BASE64"');
            await modal.getByRole('button', { name: '設定を保存' }).click({ force: true });
            await expect(modal).toBeHidden(); // 保存完了と非表示を確実に待機
            ...
            await mockPathInput.fill('window.testCustomAsyncAPI.getData');
            await mockPatternInput.fill('AsyncMock');
            const iifeCode = `(function() { return { getTracks: function() { return [{ stop: function() { window.__cameraStopped = true; } }]; } }; })()`;
            await mockResponseInput.fill(iifeCode);
            await modal.getByRole('button', { name: '設定を保存' }).click({ force: true });
            await expect(modal).toBeHidden(); // 保存完了と非表示を確実に待機
        });
        ...
        await test.step('2. プレビュー環境(iframe)内でモックが正しく機能しているか直接検証する', async () => {
            ...
            // 1. メニューが開くまで、1秒ごとにクリックと表示確認を繰り返す
            await expect(async () => {
                await platformSwitcher.locator('.screen-rotete-container').click({ force: true });
                await expect(editMenu).toBeVisible({ timeout: 500 }); // 個別タイムアウトを短めにして素早くリトライさせる
            }).toPass({
                intervals: [1000], // 1秒ごとにリトライ
                timeout: 10000     // 全体の最大タイムアウト（必要に応じて調整してください）
            });

            // メニュー内の操作
            await editMenu.getByText('動作').click();

            // 2. メニューが閉じるまで、1秒ごとにクリックと非表示確認を繰り返す
            await expect(async () => {
                await platformSwitcher.locator('.screen-rotete-container').click({ force: true });
                await expect(editMenu).toBeHidden({ timeout: 500 });
            }).toPass({
                intervals: [1000], // 1秒ごとにリトライ
                timeout: 10000     // 全体の最大タイムアウト
            });
