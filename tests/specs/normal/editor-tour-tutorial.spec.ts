...
test.describe('エディタ内：ツアーとチュートリアル機能のテスト', () => {
    ...
    test('初回起動時にツアーが表示され、完了できること', async ({ page, context, appName, appKey }) => {
        ...
        await test.step('2. エディタを起動し、ツアーが表示されることを確認', async () => {
            ...
            // =========================================================================
            // 【原因確定のための検証用インターセプト】
            // 前回のログで「.webm がリクエストされた直後にブラウザがクラッシュする」
            // ことが判明しました。これを100%確定させるため、webmの通信を監視・遮断します。
            // ※これはテストの修正ではなく、クラッシュ原因を断定するための「対照実験」です。
            // =========================================================================
            await editorPage.route('**/*.webm', async route => {
                console.log(`[TourTest:DEBUG:Video] WebM動画の読み込みリクエストを検知: ${route.request().url()}`);
                console.log(`[TourTest:DEBUG:Video] クラッシュ原因特定のため、この動画の読み込みを意図的にブロックします`);
                await route.abort('blockedbyclient');
            });

            editorPage.on('request', req => {
                if (req.url().includes('.webm')) {
                    console.log(`[TourTest:DEBUG:Network] WebMリクエスト発生: ${req.url()}`);
                }
            });
        ...
        await test.step('3. ツアーを進めて完了する', async () => {
            ...
            const pingBrowser = async (tag: string) => {
                try {
                    await editorPage.evaluate(() => 'pong', { timeout: 2000 });
                    console.log(`[TourTest:DEBUG:Ping] ${tag} - 応答あり`);
                } catch (e: any) {
                    console.log(`[TourTest:DEBUG:Ping] ${tag} - 応答なし(クラッシュ): ${e.message}`);
                }
            };
            ...
            for (let i = 0; i < 15; i++) {
                ...
                try {
                    ...
                    console.log(`[TourTest:DEBUG] 待機 (600ms) しつつ、ブラウザの生死を監視します`);
                    for (let w = 1; w <= 6; w++) {
