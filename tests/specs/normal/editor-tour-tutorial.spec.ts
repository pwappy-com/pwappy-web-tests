...
test.describe('エディタ内：ツアーとチュートリアル機能のテスト', () => {
    ...
    test('初回起動時にツアーが表示され、完了できること', async ({ page, context, appName, appKey, browserName }) => {
        ...
        await test.step('3. ツアーを進めて完了する', async () => {
            if (browserName === 'webkit') {
                // ========================================================
                // 【ログ追加】 evaluate内部の実行状況を配列で返し、Playwright側で出力する
                // （ロジック自体は変更していません）
                // ========================================================
                const evalLogs = await editorPage.evaluate(async () => {
                    ...
                    for (let i = 0; i < 15; i++) {
                        ...
                        if (!tourGuide) { l(`Iter ${i}: app-tour-guide NOT FOUND by querySelector`); break; }

                        const root = tourGuide.shadowRoot || tourGuide;
                        const nextBtn = root.querySelector('.btn-next') as HTMLButtonElement | null;
                        if (!nextBtn) { l(`Iter ${i}: .btn-next NOT FOUND in root`); break; }

                        const disp = window.getComputedStyle(nextBtn).display;
                        if (disp === 'none') { l(`Iter ${i}: .btn-next display is none`); break; }

                        const btnText = nextBtn.innerText || '';
                        l(`Iter ${i}: Clicking button. Text: "${btnText}"`);
                        nextBtn.click();
                        await delay(800);
                        if (btnText.includes('完了')) { l(`Iter ${i}: Button text included '完了'. Breaking.`); break; }
                    }
