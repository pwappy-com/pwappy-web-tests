    ...
    test('初回起動時にツアーが表示され、完了できること', async ({ page, context, appName, appKey, browserName }) => {
        ...
        await test.step('3. ツアーを進めて完了する', async () => {
            ...
            if (browserName === 'webkit') {
                ...
                // =========================================================
                // 【ログ強化】
                // 検索起点（document）が原因で空振りしていたため、
                // Playwrightが捕捉済みの tourGuide 要素を直接渡してダンプさせる
                // =========================================================
                const evalLogs = await tourGuide.evaluate(async (tg: HTMLElement) => {
                    const logs: string[] = [];
                    const l = (m: string) => { logs.push(`[Evaluate] ${m}`); };
                    const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

                    l('--- START ---');
                    l(`Element tag: ${tg.tagName}, Attributes: ${Array.from(tg.attributes).map(a => `${a.name}="${a.value}"`).join(', ')}`);

                    const root = tg.shadowRoot || tg;
                    const btn = root.querySelector('.btn-next') as HTMLButtonElement | null;
                    if (btn) {
                        l('Attach diagnostic click listener to .btn-next');
                        btn.addEventListener('click', (e) => {
                            l(`>>> [EVENT] .btn-next CLICK FIRED! isTrusted: ${e.isTrusted}`);
                        });
                    } else {
                        l('.btn-next initially NOT FOUND in shadowRoot');
                        l(`ShadowRoot innerHTML dump: ${root.innerHTML.substring(0, 500)}`);
                    }

                    for (let i = 0; i < 15; i++) {
                        l(`--- Iteration ${i} ---`);
                        const r = tg.shadowRoot || tg;
                        const nextBtn = r.querySelector('.btn-next') as HTMLButtonElement | null;
                        if (!nextBtn) {
                            l('.btn-next NOT FOUND -> break');
                            break;
                        }

                        const displayStyle = window.getComputedStyle(nextBtn).display;
                        l(`.btn-next CSS display: ${displayStyle}`);
                        if (displayStyle === 'none') {
                            l('.btn-next is display:none -> break');
                            break;
                        }

                        const btnText = nextBtn.innerText || nextBtn.textContent || '';
                        l(`.btn-next Text: "${btnText.trim()}"`);

                        l('Calling nextBtn.click() ...');
                        nextBtn.click();

                        await delay(800);

                        if (btnText.includes('完了')) {
                            l('Text included "完了" -> break');
                            break;
                        }
                    }
                    l('--- END ---');
            ...
            // =========================================================
            // 【ログ強化】
            // アサーション前に、DOM上の実際の状態をダンプする
            // =========================================================
            const beforeAssertVisible = await tourGuide.evaluate((tg: HTMLElement) => tg.getAttribute('visible'));
            logTime(`[Pre-Assert Dump] tourGuide visible attribute is currently: "${beforeAssertVisible}"`);

            try {
                await expect(tourGuide).not.toHaveAttribute('visible', '', { timeout: 15000 });
                logTime('tourGuide 非表示待機完了');
            } catch (e) {
                logTime(`[DEBUG FATAL] tourGuide did not hide! Dumping HTML state...`);
                const htmlDump = await tourGuide.evaluate(el => el.outerHTML);
                logTime(`[Dump] ${htmlDump.substring(0, 1000)}`);
                throw e;
            }
