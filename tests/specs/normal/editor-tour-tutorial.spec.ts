    ...
    test('初回起動時にツアーが表示され、完了できること', async ({ page, context, appName, appKey, browserName }) => {
        ...
        await test.step('2. エディタを起動し、ツアーが表示されることを確認', async () => {
            ...
            if (browserName === 'webkit') {
                // =========================================================================
                // 【ワークアラウンド】WebKit環境での動画デコーダーによるクラッシュ回避
                // WebKit環境において .webm 動画の読み込みを行うと、OSレベルで
                // Segmentation fault が発生しブラウザが消滅するバグがあることがログから確定しました。
                // テストの進行に影響のない動画リソースの読み込みのみをブロックし、操作は本道で続行します。
                // =========================================================================
                console.warn('[Workaround] WebKitの動画デコーダークラッシュを回避するため、.webmファイルの読み込みをブロックします。');
