...
test.describe('エディタ内：ツアーとチュートリアル機能のテスト', () => {
    ...
    test('初回起動時にツアーが表示され、完了できること', async ({ page, context, appName, appKey }) => {
        ...
        await test.step('2. エディタを起動し、ツアーが表示されることを確認', async () => {
            ...
            // =========================================================================
            // 【監視用ログ】 ネットワークリクエストとコンソールを監視し、クリック後に何が起きているか追跡
            // =========================================================================
            editorPage.on('request', req => console.log(`[TourTest:DEBUG:Network] Req: ${req.method()} ${req.url()}`));
            editorPage.on('console', msg => console.log(`[TourTest:DEBUG:Console] ${msg.type()}: ${msg.text()}`));
        ...
        await test.step('3. ツアーを進めて完了する', async () => {
            ...
            // ブラウザが生きているか（フリーズしていないか）を確認するPing関数
            const pingBrowser = async (tag: string) => {
                try {
                    const res = await editorPage.evaluate(() => 'pong', { timeout: 2000 });
                    console.log(`[TourTest:DEBUG:Ping] ${tag} - 応答あり: ${res}`);
                } catch (e: any) {
                    console.log(`[TourTest:DEBUG:Ping] ${tag} - 応答なし(フリーズ/クラッシュ): ${e.message}`);
                }
            };

            for (let i = 0; i < 15; i++) {
                console.log(`\n[TourTest:DEBUG] === ループ ${i}回目開始 ===`);

                await pingBrowser(`ループ ${i}回目開始直後`);

                try {
                    // Locator経由で確実に対象要素（ShadowDOM内）の情報をダンプする
                    const domState = await tourGuide.evaluate((tg: HTMLElement) => {
                        const isVisibleAttr = tg.getAttribute('visible');
                        const root = tg.shadowRoot || tg;
                        const btn = root.querySelector('.btn-next') as HTMLElement;

                        if (!btn) return { tourGuideVisibleAttr: isVisibleAttr, error: '.btn-next not found' };
                    ...
                    if (domState.error && domState.error.includes('failed')) {
                        console.log(`[TourTest:DEBUG] DOM状態取得で致命的エラー。これ以上進めません。`);
                        break;
                    }

                    console.log(`[TourTest:DEBUG] nextBtn.innerText() を実行`);
                    const btnText = await nextBtn.innerText({ timeout: 5000 });
                    console.log(`[TourTest:DEBUG] ボタンテキスト: "${btnText}"`);

                    console.log(`[TourTest:DEBUG] nextBtn.click() を実行`);
                    await nextBtn.click({ timeout: 5000 });
                    console.log(`[TourTest:DEBUG] クリック成功`);

                    await pingBrowser(`クリック完了直後`);

                    console.log(`[TourTest:DEBUG] 待機 (600ms) しつつ、ブラウザの生死を100msごとに監視します`);
                    for (let w = 1; w <= 6; w++) {
                        await editorPage.waitForTimeout(100);
                        await pingBrowser(`待機 ${w * 100}ms経過`);
                    }
