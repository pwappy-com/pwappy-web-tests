    ...
    test('初回起動時にツアーが表示され、完了できること', async ({ page, context, appName, appKey, browserName }) => {
        ...
        await test.step('3. ツアーを進めて完了する', async () => {
            if (browserName === 'webkit') {
                // =========================================================================
                // 【ワークアラウンド】WebKit環境でのフリーズ回避
                // WebKitにおいて、アニメーション中の要素に対してPlaywrightのLocator API
                // (innerTextやclick等のヒットテスト) を連続で呼び出すと、レンダリングエンジンが
                // デッドロックを起こし count() すら応答しなくなるバグを回避するため、
                // ブラウザ内部のJS(evaluate)としてツアー進行を完結させます。
                // =========================================================================
                console.log(`[Workaround] WebKit環境のデッドロックバグを回避するため、evaluate内部でツアーを進行させます。`);
                await editorPage.evaluate(async () => {
                    ...
                    for (let i = 0; i < 15; i++) {
                        ...
                        nextBtn.click();
                        await delay(800); // UIアニメーションの完了を待つ
