...
test.describe('クイックプロパティエディタ（QuickPropertyEditor）の高度な連動テスト', () => {
    ...
    test('基本情報編集: 保存を押さずに要素を切り替えた時の「一括自動保存」と「ID重複防止ガード」が機能すること', async ({ editorPage, editorHelper }) => {
        ...
        await test.step('3. 確定せずにプレビュー上の要素2（ボタン）を選択し、自動で一括保存されていることを検証', async () => {
            ...
            await editorPage.locator('#renderzone').click({ force: true, position: { x: 5, y: 5 } }).catch(() => { });
            ...
            await expect(savedButton1Node).toHaveAttribute('data-node-dom-id', `${node1TargetId}-edited`, { timeout: 15000 });
        ...
        await test.step('4. 異常系（ID重複ガード）: 再び要素1の編集を開き、要素2と「同じID」へ書き換えてプレビュー切り替え', async () => {
            ...
            await expect(finalButton1Node).toHaveAttribute('data-node-dom-id', `${node1TargetId}-edited`, { timeout: 15000 });

            // 検証: ただしID以外の「説明（explain）」は重複していないため、正常に自動更新されていること
            await expect(finalButton1Node.locator('.label-explain').first()).toHaveText('重複によりIDだけ保存されないはずの説明', { timeout: 15000 });
