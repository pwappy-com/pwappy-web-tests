...
// =========================================================================
// Merged from: tests/specs/normal/editor-navigation.spec.ts (後半部)
// =========================================================================

test.describe('エディタ内機能のテスト (後半)', () => {
...
customElements.define('${tagName}', ${scriptName});
    ...
    test('アプリアイコン設定（AppIconEditor）：エディタ画面内のアプリ設定パネルにおける、初期表示、宛先アコーディオン、画像アップロード、マスク形状切り替え、宛先編集と適用保存を検証する', async ({ editorPage, editorHelper }, testInfo) => {
        ...
        // --- 3. 初期表示の検証 ---
        // サーバー側がテンプレートとしてデフォルトアイコンを事前に配備している場合、
        // 起動直後にその画像が自動でロードされるため、プレースホルダーではなくキャンバスが直ちに表示されます。
        // 環境による差異（Flaky）を完全に回避するため、プレースホルダーまたはキャンバスの「どちらか一方」が可視であることを検証します。
        ...
        await expect(maskPreview).toHaveClass(/ios/); // デフォルトは iOS 角丸

        await maskSelector.locator('.mask-btn', { hasText: 'Android' }).click();
        await expect(maskPreview).toHaveClass(/android/); // Android 円形
        ...
        await expect(applyBtn).toBeEnabled(); // スケール変更検知により一括適用ボタンが活性化

        // --- 8. 宛先出力先設定の動的追加・編集・削除の検証 ---
        ...
        // --- 9. トリミング適用保存とエディタ画面内のアラートハンドリング ---
        ...
        if (alertMessage.includes('制限されています') || alertMessage.includes('デモモード')) {
            ...
            await expect(alertDialog).toContainText('トリミング更新しました');
            await alertDialog.getByRole('button', { name: '閉じる' }).click();
        }
