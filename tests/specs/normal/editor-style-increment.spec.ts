...
test.describe('CSSエディタ：数値のインテリジェント増減機能のテスト', () => {
    ...
    async function testIncrement(
        ...
        // =========================================================================
        // 【原因究明ログ】 エラーが発生する前後の状態をブラウザ内部から直接ダンプします
        // =========================================================================
        const preState1 = await editorPage.evaluate(() => {
            const container = document.querySelector('app-container');
            const host = container?.shadowRoot?.querySelector('property-container') as any;
            return {
                step: 'setValue直前',
                hostExists: !!host,
                styleEditorExists: host ? !!host.styleEditor : false,
                styleEditorType: host ? typeof host.styleEditor : 'none',
                // hostオブジェクト内に "editor" または "style" という名前が含まれるプロパティを全検索する
                availableProps: host ? Object.keys(host).filter(k => k.toLowerCase().includes('editor') || k.toLowerCase().includes('style')) : []
            };
        });
        console.log(`[StyleInc:DEBUG] ${JSON.stringify(preState1)}`);
        ...
        // =========================================================================
        // 【原因究明ログ】 エラー発生直前の状態を安全に取得します
        // =========================================================================
        const finalState = await editorPage.evaluate(() => {
            const container = document.querySelector('app-container');
            const host = container?.shadowRoot?.querySelector('property-container') as any;
            try {
                const val = (host && host.styleEditor) ? host.styleEditor.getValue() : null;
                return {
                    step: 'getValue直前（安全に取得）',
                    status: 'success',
                    hostExists: !!host,
                    styleEditorExists: host ? !!host.styleEditor : false,
                    styleEditorType: host ? typeof host.styleEditor : 'none'
        ...
        console.log(`[StyleInc:DEBUG] ${JSON.stringify(finalState)}`);

        // ！！！ここで本来のエラー（TypeError）が再現します！！！
        // 修正せずにそのまま残し、直前のログ出力によって原因を特定します。
    ...
    test('数値以外の場所では標準の行移動が行われること', async ({ editorPage }) => {
        ...
        // 【原因究明ログ】
        // =========================================================================
        const state1 = await editorPage.evaluate(() => {
            const container = document.querySelector('app-container');
            const host = container?.shadowRoot?.querySelector('property-container') as any;
            return {
                step: '数値以外テスト: setValue直前',
                hostExists: !!host,
                styleEditorExists: host ? !!host.styleEditor : false
            };
        });
        console.log(`[StyleInc:DEBUG] ${JSON.stringify(state1)}`);
        ...
        // 【原因究明ログ】
        // =========================================================================
        const state2 = await editorPage.evaluate(() => {
            const container = document.querySelector('app-container');
            const host = container?.shadowRoot?.querySelector('property-container') as any;
            return {
                step: '数値以外テスト: getPosition直前',
                hostExists: !!host,
                styleEditorExists: host ? !!host.styleEditor : false
            };
        });
        console.log(`[StyleInc:DEBUG] ${JSON.stringify(state2)}`);
