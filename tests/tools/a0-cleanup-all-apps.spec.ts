...
import { gotoDashboard } from './dashboard-helpers';
...
test.describe.serial('手動実行: 全アプリケーション削除スクリプト', () => {
  ...
  test('すべてのアプリケーションを削除する', async ({ page, context }) => {
    test.setTimeout(600000); // タイムアウトを10分に延長

    await test.step('ログインとページ遷移', async () => {
      await gotoDashboard(page);
      await expect(page.getByRole('heading', { name: 'アプリケーション一覧', level: 1 })).toBeVisible();
    });
    ...
    while (true) {
      ...
      // 名前だけでなくキーも取得する
      const appName = await firstAppRow.locator('.app-name').innerText();
      const appKey = await firstAppRow.locator('.app-key').innerText();
      console.log(`[Workbench] 処理中のアプリ: ${appName} (${appKey})`);

      const deleteButton = firstAppRow.getByTitle('アプリを削除');
      ...
      if (await deleteButton.isEnabled()) {
        ...
        await test.step(`[非公開化] ${appName} の全バージョンを非公開にします`, async () => {
          ...
          while (true) {
            ...
            await publishedVersionRow.getByRole('button', { name: /非公開/ }).click();
            await expect(page.getByText('処理中...')).toHaveCount(0, { timeout: 30000 });
        ...
        await test.step(`[再削除] ${appName} を削除します`, async () => {
          ...
          const enabledDeleteButton = appRowWorkbench.getByTitle('アプリを削除');
      ...
      // appName ではなく appKey で対象を特定する
      const deletedAppRow = page.locator('.app-card', { hasText: appKey });
    ...
    while (true) {
      ...
      const deleteButton = firstArchivedRow.getByTitle('完全に削除');
