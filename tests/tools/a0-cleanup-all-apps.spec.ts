...
test.describe.serial('手動実行: 全アプリケーション削除スクリプト', () => {
  ...
  test('すべてのアプリケーションを削除する', async ({ page, context }) => {
    ...
    while (true) {
      ...
      const deleteButton = firstArchivedRow.locator('.btn-danger-outline');
      const openButton = firstArchivedRow.getByText('OPEN');
      ...
      if (await !openButton.isVisible()) {
        ...
        await test.step(`[復元 & 非公開化] ${appName} を復元して非公開にします`, async () => {
          ...
          await firstArchivedRow.getByRole('button', { name: /復元/ }).click();
          await expect(page.getByText('処理中...')).toHaveCount(0, { timeout: 30000 });
      ...
      await test.step(`[非公開化] ${appName} の全バージョンを非公開にします`, async () => {
        while (true) {
          ...
          if (await publishedVersionRow.count() === 0) {
            break;
            // 以前あった到達不能な不要コード（break後のclick処理）は削除しました
          }

          const version = await publishedVersionRow.locator('.v-version').innerText();
          console.log(`  -> バージョン ${version} を非公開にします`);

          // Playwrightのアイコンテキスト認識揺れを防ぐため、正規表現で「非公開へ」にマッチさせる
          const unPublishBtn = publishedVersionRow.getByRole('button', { name: /非公開へ/ });
          await unPublishBtn.click();

          await expect(page.locator('message-box#publish-action-confirm')).toBeVisible({ timeout: 30000 });
