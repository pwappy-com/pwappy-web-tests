...
test.describe.serial('手動実行: 全アプリケーション削除スクリプト', () => {
  ...
  test('すべてのアプリケーションを削除する', async ({ page, context }) => {
    ...
    while (true) {
      ...
      const appKey = await firstAppRow.locator('td').nth(1).innerText();
      ...
      if (await deleteButton.isEnabled()) {
        ...
        await test.step(`[非公開化] ${appName} の全バージョンを非公開にします`, async () => {
          ...
          const appRowPublish = page.locator('.app-list tbody tr', { hasText: appKey });
        ...
        await test.step(`[再削除] ${appName} を削除します`, async () => {
          ...
          // appName ではなく appKey で対象を特定する
          const appRowWorkbench = page.locator('.app-list tbody tr', { hasText: appKey });
      ...
      // appName ではなく appKey で対象を特定する
      const deletedAppRow = page.locator('.app-list tbody tr', { hasText: appKey });
    ...
    while (true) {
      ...
      const appKey = await firstArchivedRow.locator('td').nth(1).innerText();
      ...
      await expect(page.getByText('処理中...')).toHaveCount(0, { timeout: 30000 });
      // appName ではなく appKey で対象を特定する
      const deletedArchivedRow = page.locator('.app-list tbody tr', { hasText: appKey });
